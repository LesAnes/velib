...
def get_stationarity_penalty(station, is_departure: bool):
    ...
    if (station[target_column] / station['capacity']) > 0.33:
        return 0
    station_status = get_historical_data(station, 6 * 6)
...
def get_historical_data(station, last: int) -> pd.DataFrame:
    ...
    return pd.DataFrame(get_station_status(station_id)[-last:])


def get_forecast(station, last: int, delta_hours: int = 1, is_departure: bool = True):
    station_status = get_historical_data(station, last)

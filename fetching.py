...
from db import (
    get_stations_status_collection,
    get_stations_last_state_collection,
    update_station_last_state,
    remove_old_status,
)
...
def main():
    ...
    for station in stations:
        # Api mapping
        station["mechanical"] = station.get("num_bikes_available_types")[0].get(
            "mechanical"
        )
        ...
        status_by_station_id = stations_status_col.find(
            {"station_id": station.get("station_id")}
        )
        status_by_station_id_sorted = status_by_station_id.sort("last_reported", -1)
        last_reported = (
            status_by_station_id_sorted[0].get("last_reported")
            if status_by_station_id_sorted.count() > 0
            else None
        )

        if last_reported is None or int(station.get("last_reported")) > int(
            last_reported
        ):
